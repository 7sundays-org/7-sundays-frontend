...
export const TestimonialCard: FC<{ data: TestimonialData }> = ({ data }) => (
  <article className="flex flex-col gap-4 md:gap-[42px]">
    ...
    <div className="line-clamp-3 font-sans text-[1.5625rem] leading-[1.4] font-bold italic text-white md:text-[40px] md:leading-[48px] md:not-italic">
...
export const TestimonialsCarousel: FC<{ items: TestimonialData[] }> = ({
  ...
  return (
    <div className="relative">
      ...
      {count > 1 && (
        <>
          {/* Dots — mobile only, centrati */}
          <div className="mt-8 flex justify-center items-center gap-2.5 md:hidden">
            {items.map((_, i) => (
              <button
                key={i}
          ...
          <CarouselArrow
            ...
            disabled={active === 0}
            className="hidden md:flex absolute top-1/2 -left-6 z-10 -translate-y-1/2 lg:-left-[100px]"
          />
          <CarouselArrow
            direction="right"
            ...
            className="hidden md:flex absolute top-1/2 -right-6 z-10 -translate-y-1/2 lg:-right-[100px]"
          />
        </>
      )}
    </div>
